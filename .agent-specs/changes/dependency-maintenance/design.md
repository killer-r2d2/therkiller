...
5. Confirm that `npm ls vue vue-router nuxt` contains one intended Vue and Vue Router line rather than parallel Router 4 and Router 5 installations.
6. Review the Nuxt 4.5 build-layer changes against this repository's custom Vite Tailwind plugin and `future.compatibilityVersion: 5` setting.
...
- `package.json`: version ranges, removal of `lint-staged`, and removal of its unused configuration.
